...
![システム設定のキャプチャ](./setup-preference.webp?size=1280x892)

| 画面遷移 | 変更内容と手順 | 備考 |
| :- | :- | :- |
| アクセシビリティ → ディスプレイ | 「ポインタ」の「カーソルのサイズ」を少し上げる |  |
| ディスプレイ | 「スペースを拡大」 | メモリが8GBなどの機種では負荷が増すため非推奨 |
| メニューバー | 任意の項目の表示設定を「使用中に表示」から「常に表示」に変更 | Bluetooth、バッテリー、集中モード |
| メニューバー → バッテリーオプション | 「割合(%)を表示」を有効化 |  |
| 外観 | 「スクロールバーの表示」を「常に表示」 |  |
| キーボード → 「テキスト入力」内「入力ソース」の「編集」→ すべての入力ソース | 「文頭を自動的に大文字にする」を無効化、<br>「スペースバーを2回押してピリオドを入力」を無効化 |  |
| キーボード → 「テキスト入力」内「入力ソース」の「編集」→ 日本語 - ローマ字入力 | 「ライブ変換」を無効化、<br>「Windows風のキー操作」を**有効化** |  |
| キーボード → キーボードショートカット → キーボード | 「次のウインドウを操作対象にする」を`Option` + `Tab`に変更 | デフォルトは`Option` + `@` |
| トラックパッド → ポイントとクリック | 「強めのクリックと触覚フィードバック」を無効化 |
